...
6. Com a escala aplicada, a área da direita levanta os 16 cômodos traçados. **Andar por dentro** entra no passeio: W A S D ou setas para caminhar, mouse para olhar, Shift para acelerar, Esc para sair. Durante o passeio, uma seta laranja na planta mostra onde você está e para onde olha. **Enquadrar**, **Vista superior** e **Ocultar paredes** ajudam na visão geral. **Cores: acabamento** pinta piso e parede com os materiais lidos nas fotografias; o mesmo botão alterna para **Cores: conferência**, onde piso bege é cômodo conferido por cota impressa e piso azulado é cômodo sem cota para conferir. **Com mobília** alterna entre o apartamento mobiliado e o vazio; sem mobília, a circulação volta a ser exatamente a de antes. **Nomes no chão**, ligado por padrão, escreve no piso o nome de cada cômodo e sua área — útil na visão de cima. **Medidas** desenha a cota de cada parede, com setas nas extremidades e o comprimento no meio: são as medidas do traçado, não as cotas impressas na planta. Dentro do passeio, o nome do cômodo aparece no alto das paredes e o destino sobre cada porta, que é o que orienta quem caminha. Clicar no modelo entra no passeio; arrastar continua girando a vista. Dentro do passeio, segurar o botão do mouse anda para a frente, como a tecla W. **Tela cheia** amplia o painel inteiro, com a barra de botões junto, e dentro do passeio a tecla **F** faz o mesmo — com o ponteiro capturado, nenhum botão é clicável. A roda do mouse aproxima e afasta, e o leque na planta acompanha a vista em tempo real. Em tela de toque o passeio tem controles próprios: arrastar o dedo olha em volta, quatro setas no canto da cena andam, e a pinça de dois dedos aproxima e afasta — não há ponteiro para prender nem teclado à mão.
7. **Experimento visual do LIVING** é um painel local do editor, fechado por padrão, que existe para comparar aparências no mesmo enquadramento. Três opções: **Atual**, sem nada aplicado; **Acabamentos**, que troca os materiais do living por PBR, desenha o parquete em espinha, acende o teto e o rodapé — e não toca em nenhuma configuração global, então os outros cômodos ficam idênticos; e **Iluminação**, que acrescenta curva de tom, exposição ajustável, sombras e um cenário **ilustrativo** de sol entrando pelas aberturas da fachada sul. Curva de tom e luzes são globais: nesse terceiro modo **os outros cômodos também mudam**. Trocar de opção não move a câmera, que é o que torna as três comparáveis. Para comparar no recorte da fotografia, selecione `foto_sala_03`, clique em **Comparar com o modelo** e alterne as opções. O painel mostra draw calls, triângulos e o tempo de quadro — este só durante o passeio, porque fora dele a cena não desenha. O experimento não convive com **Cores: conferência** e se suspende sozinho quando ela é ligada. Ele existe só aqui: `npm run build:visita` não o inclui na imagem publicada. Roteiro de revisão em `docs/40_delivery/F4_EXPERIMENTO_LIVING.md`.
8. **Exportar JSON** baixa `vovo-bisa-projeto.json`. Guarde uma cópia; para versionar a nova base, salve em `src/data/project.json`. O navegador não escreve automaticamente nessa pasta.
9. **Importar JSON** valida integralmente o arquivo antes de aplicá-lo. Arquivos inválidos mantêm o estado anterior. A importação válida substitui o rascunho da sessão.
...
- `src/scene/parquet.ts`: padrão em espinha de peixe gerado por regra, puro e sem dependência de tela.
- `src/scene/experiment.ts`: experimento visual do living. Instanciado só pelo editor, então não entra no pacote da visita.
