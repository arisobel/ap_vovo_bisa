  ...
  <main>
    ...
    <section class="workspace" aria-label="Planta e visualização">
      ...
      <article class="panel visual-panel">
        ...
        <div class="plan-tools" id="scene-tools" hidden><span id="scene-info"></span><div><button id="scene-walk" class="primary">Andar por dentro</button><button id="scene-frame">Enquadrar</button><button id="scene-top">Vista superior</button><button id="scene-walls" aria-pressed="true">Ocultar paredes</button><button id="scene-palette" aria-pressed="true">Cores: acabamento</button></div></div>
...
Promise.all([import('./scene/preview'), import('./data/pilot')]).then(([preview, pilot]) => {
  ...
  scene.setWalkListener(estado => {
    ...
    for (const id of ['scene-frame', 'scene-top', 'scene-walls', 'scene-palette']) el<HTMLButtonElement>(id).disabled = ativo;
